    ...
    cudaStream_t stream, int* occupancy = nullptr)
{
    ...
    case tkc::EpilogueScheduleType::AUTO:
        ...
        sm100_generic_mixed_gemm_kernelLauncher<ActivationType, WeightType, ScaleZeroType, BiasType, OutputType, QuantOp,
    ...
    default:
        throw std::runtime_error(
            "[TensorRT-LLM Error][fpA_intB][sm100_dispatch_epilogue_schedules] epilogue schedule config is invalid for "
            "mixed "
            "type GEMM.");
        break;
    }
    ...
    cudaStream_t stream, int* occupancy = nullptr)
{
    ...
    if constexpr (tile_shapes_supported)
    {
        ...
        default:
            throw std::runtime_error(
                "[TensorRT-LLM Error][fpA_intB][sm100_dispatch_mainloop_schedules] mainloop schedule config is invalid "
                "for "
                "mixed type GEMM.");
            break;
        }
    ...
    {
        throw std::runtime_error(
            "[TensorRT-LLM Error][fpA_intB][sm100_dispatch_mainloop_schedules] Unsupported CTA and Cluster shapes for "
            "mixed type GEMM.");
    }
}
    ...
    cudaStream_t stream, int* occupancy = nullptr)
{
    ...
    {
    case tkc::CutlassTileConfigSM100::CtaShape64x16x128B:
        sm100_dispatch_gemm_config<ActivationType, WeightType, ScaleZeroType, BiasType, OutputType, QuantOp, EpilogueTag,
    ...
    case tkc::CutlassTileConfigSM100::Undefined:
        throw std::runtime_error(
            "[TensorRT-LLm Error][fpA_intB][sm100_dispatch_gemm_to_cutlass] gemm config undefined.");
        break;
    case tkc::CutlassTileConfigSM100::ChooseWithHeuristic:
        throw std::runtime_error(
            "[TensorRT-LLm Error][fpA_intB][sm100_dispatch_gemm_to_cutlass] gemm config should have already been set by "
            "heuristic.");
        break;
    default:
        TLLM_LOG_ERROR("Unsupported tile configuration");
        break;
    }
}
