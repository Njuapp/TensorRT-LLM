...
template <typename TypeA, typename Arch>
    ...
    uint8_t, Arch,
    typename platform::enable_if<Arch::kMinComputeCapability<=90 || Arch::kMinComputeCapability >= 120 >::type>
{
    static constexpr int ThreadblockK = 128 * 8 / cutlass::sizeof_bits<TypeA>::value;
...
template <typename TypeA, typename Arch>
struct LayoutDetailsB<TypeA, uint8_t, Arch, typename platform::enable_if<Arch::kMinComputeCapability >= 100 && Arch::kMinComputeCapability < 120>::type>
{
    static constexpr int ThreadblockK = 128 * 8 / cutlass::sizeof_bits<TypeA>::value;
    using Layout = layout::ColumnMajor;
    static constexpr int ElementsPerAccess = 128 / cutlass::sizeof_bits<half_t>::value;
...
template <typename TypeA, typename Arch>
struct LayoutDetailsB<TypeA, uint4b_t, Arch, typename platform::enable_if<Arch::kMinComputeCapability >= 100 && Arch::kMinComputeCapability < 120>::type>
{
    static constexpr int ThreadblockK = 128 * 8 / cutlass::sizeof_bits<TypeA>::value;
    using Layout = layout::ColumnMajor;
    static constexpr int ElementsPerAccess = 128 / cutlass::sizeof_bits<half_t>::value;
